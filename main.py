...
def display_data():
  ...
  draw_text(str(world.health), text_font, "white", c.SCREEN_WIDTH + 50, 40)
  screen.blit(coin_image, (c.SCREEN_WIDTH + 10, 65))
  draw_text(str(world.money), text_font, "white", c.SCREEN_WIDTH + 50, 70)
  ...
  if game_over == False:
    ...
    if placing_turrets == True:
      ...
      if cancel_button.draw(screen):
        placing_turrets = False
    #if a tower is selected then show the upgrade button
    if selected_turret:
  ...
  for event in pg.event.get():
    ...
    if event.type == pg.MOUSEBUTTONDOWN and event.button == 1:
      ...
      if mouse_pos[0] < c.SCREEN_WIDTH and mouse_pos[1] < c.SCREEN_HEIGHT:
        ...
        clear_selection()
        if placing_turrets == True:
          #check if there is enough money for a tower
